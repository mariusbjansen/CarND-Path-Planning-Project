      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // in with more points that control
          vector<double> ptsx;
          vector<double> ptsy;
          ...
          // if previous size is almost empty, us ethe car as starting reference
          if (prev_size < 2) {
          ...
          // use the previous path's end points as starting reference
            ...
            // redine reference state as previous path endpoint
          ...
          // ptsx should be 5 now (the previous two points plus evenly spaced
          // 30, 60, 90 m ahead)
          for (int i = 0; i < ptsx.size(); i++) {
            // shift car reference angle to 0 degrees -> so starting at origin
            // (x,y,yaw) =(0,0,0)
          ...
          // define the actualy (x,y) points we will use for the planner
          vector<double> next_x_vals;
          vector<double> next_y_vals;

          // Start with all of the previous path points from last time
          // NOTE: Only the previous path which was not driven is being stored
          // in previous_path!
          // So when out of initial 50 waypoints only 7 have been driven through
          // 43 will be in previous_path
